...
interface HospitalListItem {
  ...
  languages: string[];
  primaryAddress?: HospitalAddress;
}

async function fetchHospitals(): Promise<HospitalListItem[]> {
  try {
    ...
    const json = (await response.json()) as IResponse<IPagination<HospitalListItem>>;
...
function formatAddress(address?: HospitalAddress): string {
  if (!address) {
    return 'Ghana';
  }
                ...
                <div className="flex flex-col gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                      {formatAddress(hospital.primaryAddress)}
                    </p>
                  </div>
